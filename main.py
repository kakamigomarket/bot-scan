...
import os, time, asyncio, logging, traceback
...
async def _check_auth(update: Update) -> bool:
    ...
    if ALLOWED_USERS and uid not in ALLOWED_USERS:
        await update.message.reply_text("⛔ Akses ditolak. Kamu tidak terdaftar sebagai pengguna.", reply_markup=kb_main())
        return False
    return True
...
async def ping(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("pong ✅", reply_markup=kb_main())
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🟢 Retail Mode":
        ...
        await update.message.reply_text("Retail Mode dipilih. Pilih strategi:", reply_markup=kb_mode("retail"))
        return

    if text == "🧠 Pro Mode":
        context.user_data["mode"] = "pro"
        await update.message.reply_text("Pro Mode dipilih. Pilih strategi:", reply_markup=kb_mode("pro"))
        return
    ...
    if text == "⬅️ Kembali":
        context.user_data["mode"] = None
        await update.message.reply_text("Kembali ke menu utama.", reply_markup=kb_main())
        return

    if text in ("🔴 Jemput Bola","🟡 Rebound Swing","🟢 Scalping Breakout"):
        mode = context.user_data.get("mode")
        if mode not in ("retail","pro"):
            await update.message.reply_text("Pilih mode dulu ya.", reply_markup=kb_main()); return
        await update.message.reply_text(f"🔍 [{mode.upper()}] Memindai sinyal untuk strategi {text}...\nTunggu beberapa saat...", reply_markup=kb_mode(mode))
        await run_scan(update, context, text, mode_profile=mode)
        await update.message.reply_text("Selesai. Kembali ke menu utama.", reply_markup=kb_main())
        return
...
async def run_scan(update: Update, context: ContextTypes.DEFAULT_TYPE, strategy_name: str, mode_profile: str="retail"):
    ...
    async with aiohttp.ClientSession(headers={"User-Agent": "SignalBot/1.0"}) as session:
        ...
        async def pv(pair: str):
            try:
                ...
                logging.info(f"skip {pair}: {e}"); return pair, None, None
    ...
    if messages:
        ...
        await update.message.reply_text("✅ Scan selesai. Sinyal layak ditemukan.")
    else:
