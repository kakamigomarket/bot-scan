...
PAIRS = [
    ...
    "CAKEUSDT","WIFUSDT","KAIAUSDT","LDOUSDT","NEXOUSDT","XTZUSDT","SUSDT","VIRTUALUSDT",
    "AUSDT","THETAUSDT","IOTAUSDT","JASMYUSDT","RAYUSDT","GALAUSDT","DEXEUSDT","SANDUSDT","PENDLEUSDT"
]
...
class BinanceClient:
    ...
    async def quote_volume_24h(self, symbol: str) -> float:
        ...
        return float(t["data"].get("quoteVolume", 0.0))
...
async def analisa_strategi_pro(client: BinanceClient, symbol: str, strategy_name: str, price: float, vol24h: float, tf_interval: str, market_trend: str) -> Optional[str]:
    try:
        ...
        lines = []
        lines.append(f"{strategy_name} • {tf_interval}")
        lines.append(f"✅ {symbol}")
        lines.append(f"Harga: ${price:.6f}")
        lines.append(f"EMA7: {ema7:.6f} | EMA25: {ema25:.6f} | EMA99: {ema99:.6f}")
        lines.append(f"RSI(6): {rsi_last} | ATR(14): {atr14:.6f} (ATR%: {atr_pct:.2f}%)")
        lines.append(f"ADX(14): {adx_val:.2f}")
        lines.append(f"📈 24h QuoteVol: ${vol24h:,.0f}")
        lines.append(f"📉 Avg TF Vol (base): {avg_tf_vol:,.2f}")
        lines.append(f"🧪 Avg Trades/Candle: {avg_tf_trades:,.0f}{mtf_note}")
        lines.append("")
        lines.append(f"🎯 Entry: ${price:.6f}")
        lines.append(f"🎯 TP1: ${tp1} (+{tp1_pct}%)")
        lines.append(f"🎯 TP2: ${tp2} (+{tp2_pct}%)")
        lines.append(f"🛡️ SL (ATR): ${sl_atr}")
        lines.append(f"🎯 Confidence Score: {round(score,2)}/5")
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("scan:"):
        ...
        await run_scan(update, context, strategy, mode_profile=mode)
        return
